...
- `SPEAK_MODEL` — boot-default ElevenLabs model id for single-voice synthesis (default: `eleven_v3`). Runtime precedence is `config.json` > `SPEAK_MODEL` > `eleven_v3`, and every source is validated so the advertised model is always one the hop chain routes; the UIs flip it through `GET`/`POST /config`, which persists to the gitignored `config.json`. Under `SPEAK_STREAMING=0` the conversational model is refused with 409 — the legacy path can only synthesize `eleven_v3`.
